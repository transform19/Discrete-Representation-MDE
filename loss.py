...
class total_loss(nn.Module):
    def __init__(self):
        ...
        self.silog_loss = silog_loss(0.85)
        self.tv_loss = TVLoss()
        self.gradient_loss = gradient_loss()
        self.vn_loss =  VNL_Loss(1.0, 1.0, (240, 320))

    def forward(self, depth_est, depth_gt, mask):
        sil_loss = self.silog_loss(depth_est, depth_gt, mask)
        grad_loss = self.gradient_loss(depth_est, depth_gt)
        #vn_loss = self.vn_loss(10 * depth_est, 10 * depth_gt)
        #tv_loss = self.tv_loss(depth_est)

        return sil_loss + 5 * grad_loss
...
class silog_loss(nn.Module):
    ...
    def forward(self, depth_est, depth_gt, mask):
        d = torch.log(depth_est[mask] + 1e-10) - torch.log(depth_gt[mask])
        return torch.sqrt((d ** 2).mean() - self.variance_focus * (d.mean() ** 2)) * 10.0

class gradient_loss(nn.Module):
    def __init__(self, gradient_loss_weight = 1):
        super(gradient_loss, self).__init__()
        self.gradient_loss_weight = gradient_loss_weight

    def forward(self, masks_pred, true_masks):
        dx_pred, dy_pred = self.gradient(masks_pred)
        dx_true, dy_true = self.gradient(true_masks)
        gradient_loss = self.gradient_loss_weight * (torch.mean(torch.abs(dy_pred - dy_true) + torch.abs(dx_true - dx_pred)))
